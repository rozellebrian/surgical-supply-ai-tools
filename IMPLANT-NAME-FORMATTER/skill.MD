...
description: >
  Look up a surgical implant or device by reference/catalog number and return a
  standardized formatted name following the OR naming convention:
  [Item Type] [Modifiers] [Size/Spec] | [Vendor] | [Catalog #] | [HCPCS Code].
  Trigger this skill whenever the user types a reference number, catalog number,
  or part number and asks what the item is, what it's called, how to name it,
  or how to format it. Also trigger for queries like "look up ref #", "what is
  part number X", "format this item", "what do I call this", or any bare
  numeric/alphanumeric string that looks like a catalog or reference number in
  the context of surgical supply. Do NOT use this skill for HCPCS code-only
  lookups (use hcpcs-c-code-lookup) or for recall queries (use
...
## Purpose
Take a reference number (catalog number, part number, or UDI-DI) entered by
the user and return a standardized formatted item listing for use in OR
inventory records, consignment bills, implant logs, and Workday/Cerner entries.
...
1. **Material**: Titanium, Stainless Steel, PEEK, Cobalt-Chrome, Tantalum, Bioabsorbable, Allograft, Autograft, Synthetic
2. **Bone type / region** (if applicable): Cortical, Cancellous, Corticocancellous
3. **Mechanical property**: Locking, Non-Locking, Cannulated, Solid, Fenestrated, Polyaxial, Monoaxial, Expandable, Compressible
4. **Surface / treatment**: Porous, HA-Coated, Hydroxyapatite, Anodized, DBM (demineralized bone matrix)
5. **Self-tapping / Self-drilling** (screws only)
6. **Approach / orientation** (spine): TLIF, ALIF, PLIF, LLIF, XLIF, OLIF, Posterior, Anterior, Lateral
7. **Fixation type** (if relevant): Cemented, Cementless, Press-fit, Threaded
...
Web-search: `site:accessgudid.nlm.nih.gov "[reference number]"`
Or fetch: `https://accessgudid.nlm.nih.gov/devices/[number]`
...
Once item type and description are known, apply the HCPCS C Code lookup logic
(reference the hcpcs-c-code-lookup skill or the surgical-c-codes reference table).
...
1. Note "Not found in GUDID or vendor search"
2. Ask the user: vendor name, product type, or any label text visible on the package
3. Once user provides context, apply naming convention manually based on their description
4. Flag as "MANUAL ENTRY — verify with rep before entering in Cerner/Workday"
